import { ProjectEvaluation, ProjectSubmission } from '@obscuranet/shared';
import { getOpenAIConfig } from '@obscuranet/shared';
...
export async function evaluateProject(project: ProjectSubmission): Promise<ProjectEvaluation> {
  try {
    ...
    if (overallScore > 8) {
      feedback += "This is a strong project proposal with excellent potential. With proper execution, it could become a valuable addition to the Z-Origin ecosystem.\n\n";
