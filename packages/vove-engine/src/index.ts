import { VOVE_CHARACTERS, CONTRIBUTION_CATEGORIES, getEnv, ContributionAnalysis } from '@vovelet/shared';
...
const ANALYSIS_SYSTEM_PROMPT = `You are an AI analyst for VoveletAI, a platform that evaluates and categorizes contributions.
...
export async function analyzeContributionLegacy(
  contribution: string,
  character: keyof typeof VOVE_CHARACTERS = VOVE_CHARACTERS.ANALYST
): Promise<string> {
  return new Promise((resolve) => {
    setTimeout(() => {
      switch (character) {
        case VOVE_CHARACTERS.ANALYST:
        ...
        case VOVE_CHARACTERS.CRITIC:
          ...
          break;
        default:
          resolve(`Default response: Thank you for your contribution to VoveletAI.`);
      }
    }, 500);
  });
}
